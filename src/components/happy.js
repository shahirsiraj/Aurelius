...
function Happy() {
  ...
  return (
    <div style={{ height: "100vh", backgroundColor:"floralwhite"}}>
      ...
      <Grid item xs={6} >
        <Box display="flex" flexDirection="column" alignItems="center" borderRadius="32px" border={1}>
          ...
          <Box component="form" >
            ...
            />
            <Button variant="outlined" onClick={handleSubmit} color="secondary" type="submit" sx={{ mt: 2 }}>
              Add Entry
            </Button>
          </Box>
        </Box>
          ...
          <List>
            {journalEntries.map((entry, index) => (
              <ListItem key={index} backgroundColor="lavender">
                ...
                <Button variant= "outlined" color="secondary" onClick={() => handleRemove(entry)} >Remove</Button>
              </ListItem>
            ))}
          </List>
        </Box>
